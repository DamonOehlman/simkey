...
test('can simulate a keypress with a shift modifier in an element', function(t) {
  t.plan(2);

  testTarget.addEventListener('keydown', function handleKey(evt) {
    t.equal(evt.keyCode, 65, 'got expected keycode');
    t.ok(evt.shiftKey, 'shiftKey flag set');
    testTarget.removeEventListener('keydown', handleKey);
  });

  simkey(testTarget, { shift: true }, 65);
});

test('can perform partial application of simkey with shift modifier', function(t) {
  var keys = [65, 67];
  var sendKey;
  ...
  sendKey = simkey(testTarget, { shift: true });
